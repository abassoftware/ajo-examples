...
/**
 * This class shows how to use language independent texts. The translation of each
 * text is stored in a Property file with the locale as extension in the file.
 *
...
public class LanguageIndependentTexts implements ContextRunnable {
	...
	public int runFop(FOPSessionContext ctx, String[] args) throws FOPException {
		...
		new TextBox(ctx.getDbContext(),
				getTextInOperatingLanguage("LanguageIndependentTexts.1"),
				getTextInOperatingLanguage("LanguageIndependentTexts.2")).show();

		// TextBox containing variable text
		String param =
				FO.lesen(new String[] { getTextInOperatingLanguage("LanguageIndependentTexts.4") });
		new TextBox(ctx.getDbContext(),
				getTextInOperatingLanguage("LanguageIndependentTexts.1"),
				getTextInOperatingLanguage("LanguageIndependentTexts.3", param))
				.show();
	...
	/**
	 * Gets text specified by key from LanguageIndependentTexts_lang.properties in
	...
	protected String getTextInOperatingLanguage(String key, Object... params) {
		...
		Locale operatingLangLocale =
				EKS.getFOPSessionContext().getOperatingLangLocale();
		// gets text specified by key from ControlFOPCopySystem.properties in
		// previously specified operating language
		String bundle =
				ResourceBundle.getBundle(LanguageIndependentTexts.class.getName(),
						operatingLangLocale,
						LanguageIndependentTexts.class.getClassLoader()).getString(
								key);
